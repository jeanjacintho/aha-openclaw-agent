...
import { getConfig } from "../aha/config.ts";
...
test("aha_secret_set never returns or logs the token and is DM-only for the owner", async t => {
  const dir = await home(t);
  const logs: string[] = [];
  const owner = tools({ senderIsOwner: true, requesterSenderId: "plow-owner", nativeChannelId: "cht_dm" }, logs);
  await owner.get("aha_setup_save")!.execute("call", setupArgs);
  const token = "ghs_super_secret_token_value";
  const set = await owner.get("aha_secret_set")!.execute("call", { source: "github", token });
  ...
  const group = tools({ senderIsOwner: true, requesterSenderId: "plow-owner", nativeChannelId: "cht_group" });
  const denied = await group.get("aha_secret_set")!.execute("call", { source: "github", token: "other" });
