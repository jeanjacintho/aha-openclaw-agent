...
test("migration creates every table and a second open does nothing", async t => {
  ...
  assert.equal((first.db.prepare("SELECT schema_version FROM meta").get() as { schema_version: number }).schema_version, 5);
...
test("two processes can open a new database at the same time", async t => {
  ...
  for (let round = 0; round < 5; round++) {
    ...
    t.after(() => store.close());
    assert.equal((store.db.prepare("SELECT schema_version FROM meta").get() as { schema_version: number }).schema_version, 5);
