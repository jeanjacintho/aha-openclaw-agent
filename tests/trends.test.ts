...
function mention(store: Store, over: { source?: string; ext: string; published: string; topic: string }) {
  store.db.prepare(`INSERT INTO items (source, external_id, url, author, title, body, published_at, fetched_at, state)
    VALUES (?, ?, 'https://example.test/x', 'a', 't', 'plow', ?, ?, 'relevant')`).run(
    over.source ?? "hn", over.ext, over.published, over.published,
  );
  ...
  store.db.prepare(`INSERT INTO classifications (item_id, sentiment, category, topic, language, is_question, urgency, about, confidence)
    VALUES (?, 0, 'bug', ?, 'en', 0, 'med', 'self', 0.9)`).run(id, over.topic);
  return id;
}
...
test("trend alert needs current week >= 3 and >= 2x the mean of known prior weeks", async t => {
  ...
  assert.match(trendSentence(alerts[0], "pt"), /4 menções/);
  assert.match(trendSentence(alerts[0], "pt"), /HN 3/);
  assert.match(trendSentence(alerts[0], "pt"), /PH 1/);
});
